 ...
 */
@InitiatingFlow
class SubmitCompletionFlow(private val ref: StateRef, private val verdict: AccordanceStatus) : FlowLogic<StateAndRef<AccordanceState>>() {
    init {
        require(verdict in setOf(AccordanceStatus.APPROVED, AccordanceStatus.REJECTED)) {
    ...
    override fun call(): StateAndRef<AccordanceState> {
        ...
        val latestRecord = serviceHub.vaultService.queryBy<AccordanceState>(criteria).states.single()
        // DOCEND 1
        ...
        require(latestRecord.state.data.status == AccordanceStatus.NEW) {
        ...
        val tx = TransactionBuilder(notary).
                withItems(
                        latestRecord,
                        StateAndContract(newState, CONTRACT_ID),
                        Command(AccordanceContract.Commands.Completed(),
