        ...
        when (command.value) {
            is Commands.Issue -> {
                ...
                requireThat {
                    ...
                    "Initial Issue status must be NEW" using (issued.status == ProposalStatus.NEW)
                }
            }
            is Commands.Completed -> {
                ...
                for ((inputs, outputs) in stateGroups) {
                    ...
                    requireThat {
                        "Only a non-final dataAccordance can be modified" using (before.status == ProposalStatus.NEW)
